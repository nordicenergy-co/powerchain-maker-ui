<template>
  <div style="min-width: 400px;">
    <div class="flex flex-col items-center">
      <h1 class="font-powerchain text-3xl font-bold">Request withdraw deposit from chain</h1>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleAction () {
      ...
      try {
        const response = await this.ethereum.withdrawDepositInChain(this.chain, this.tokens)
        console.log(response)
        this.$store.dispatch('setWithdrawDepositInChain', {
          tokens: this.tokens,
          timestamp: new Date(),
